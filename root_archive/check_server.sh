...
if pgrep -f "whatsapp_baileys/server.js" > /dev/null; then
    echo "✅ WhatsApp connector (Baileys) is RUNNING"
    echo "   PID: $(pgrep -f whatsapp_baileys/server.js)"
else
    echo "❌ WhatsApp connector (Baileys) is NOT running"
fi
...
if [ -f ".env" ]; then
    ...
    if grep -q "BACKEND_URL" .env; then
        echo "✅ Backend URL found in .env"
    else
        echo "⚠️  BACKEND_URL might be missing in .env"
    fi
else
...
if [ $WHATSAPP_RUNNING -eq 1 ]; then
    echo "✅ WhatsApp Connector: RUNNING"
else
    echo "❌ WhatsApp Connector: NOT RUNNING"
fi
...
if [ $WHATSAPP_RUNNING -eq 1 ] || [ $BACKEND_RUNNING -eq 1 ]; then
    ...
    if screen -ls | grep -q "whatsapp\|backend"; then
        ...
        echo "  kill $(pgrep -f 'whatsapp_baileys/server.js\|uvicorn backend.app:app')"
    fi
else
    echo "To start servers:"
    echo "  node whatsapp_baileys/server.js"
fi
